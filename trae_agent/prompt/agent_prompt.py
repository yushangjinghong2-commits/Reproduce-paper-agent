...
- For PyTorch/CUDA errors, treat the root cause as an incorrect torch/torchvision/torchaudio CUDA build in the dedicated conda environment. Do not switch to CPU fallback. Record category `pytorch_cuda`, then reinstall the CUDA 12.4 package triplet, preferably `conda run -n <env> pip install --force-reinstall "torch==2.5.1+cu124" "torchvision==0.20.1+cu124" "torchaudio==2.5.1+cu124"` using the default configured pip index, verify `torch.version.cuda` is `12.4` and `torch.cuda.is_available()` is true, then rerun the failed command.
- If README or the README-referenced requirements file gives a PyTorch version, use that constraint but enforce `<2.6` and a `+cu124` CUDA build for torch, torchvision, and torchaudio. If neither gives a PyTorch version, first try `pip install "torch==2.5.1+cu124" "torchvision==0.20.1+cu124" "torchaudio==2.5.1+cu124"` from the default pip index. If it fails because that exact wheel is unavailable in the configured package mirror, choose the nearest available `<2.6` `+cu124` triplet and record the reason. Do not use custom torch index URLs unless the user explicitly changes this rule. After each requirements install, verify with `conda run -n <env> python -c 'import torch; print(torch.__version__); print(torch.version.cuda); print(torch.cuda.is_available())'`; if torch is `>=2.6`, lacks `+cu124`, reports CUDA other than `12.4`, or CUDA is unavailable, immediately reinstall the CUDA 12.4 triplet before installing flash-attn or running evaluation.
